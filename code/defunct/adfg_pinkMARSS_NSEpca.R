...
# set working dir
here::i_am("code/adfg_pinkMARSS_NSEpca.R")
...
save(strvar.df, file=here("data", "strvarEO.Rda"))
write.csv(strvar.df, here("data", "strvarEO_NSE.csv"), row.names = TRUE)

load(here("data", "strvarEO.Rda"))

# principal component analysis
pca_strvar.df <- strvar.df[, -1]
rownames(pca_strvar.df) <- strvar.df[, 1]

# # what about scaling variance terms?
# pca_scl_strvar.df <- pca_strvar.df
# pca_scl_strvar.df$sclvarE <- scale(pca_scl_strvar.df$varE)
# pca_scl_strvar.df$sclvarO <- scale(pca_scl_strvar.df$varO)
# pca_scl_strvar.df <- pca_scl_strvar.df[, -c(1:2)]

cov(pca_strvar.df)
# cov(pca_scl_strvar.df)
